...
public class CredentialsCodec {
  ...
  public String toJson(MarketplaceCredentials creds) {
    try {
      ...
      throw new IllegalStateException("Ошибка сериализации кредов", e);
    }
  }
  ...
  public MarketplaceCredentials fromJson(String json) {
    try {
      ...
      throw new IllegalStateException("Ошибка десериализации кредов", e);
    }
  }
  ...
  public String mask(MarketplaceCredentials creds) {
    if (creds instanceof WbCredentials wb) {
      return MASKERS.getOrDefault(MarketplaceType.WILDBERRIES, _c -> "***").mask(wb);
    }
    if (creds instanceof OzonCredentials oz) {
      return MASKERS.getOrDefault(MarketplaceType.OZON, _c -> "***").mask(oz);
    }
    return "***";
  }
