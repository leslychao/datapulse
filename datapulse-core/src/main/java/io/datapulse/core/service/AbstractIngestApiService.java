...
public abstract class AbstractIngestApiService<
  ...
  protected R toResponse(@NotNull D dto) {
    return mapper().to(dto, responseType());
  }

  @Transactional
  public R createFromRequest(@Valid @NotNull CR request) {
  ...
  @Transactional
  public R updateFromRequest(@NotNull Long id, @Valid @NotNull UR request) {
