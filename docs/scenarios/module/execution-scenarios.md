...
- **Main path:** `defaultRequeueRejected=false` → message consumed (ACK), не requeue'd. `outbox_event.status` → ERROR, `last_error` записан. `log.error` с `correlation_id`. Alert: «poison pill detected». Retriable business errors (HTTP 429, 503, timeout) обрабатываются через CAS → RETRY_SCHEDULED → INSERT `outbox_event` с TTL. Non-retriable ошибки → message consumed, investigation через `outbox_event.last_error`.
