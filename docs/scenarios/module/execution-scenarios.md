...
Execution отвечает за жизненный цикл `price_action` — от создания до подтверждённого результата. Управляет state machine (12 состояний, 23 перехода), transactional outbox, CAS guards, retry с backoff, reconciliation, и simulation mode.
...
- **Main path:** APPROVED → ON_HOLD (hold). ON_HOLD → APPROVED (resume). CAS guard. После resume action проходит стандартный путь APPROVED → SCHEDULED → EXECUTING.
- **Dependencies:** User role: OPERATOR+. `hold_reason` обязателен при hold.
- **Failure risks:** Action expires while on hold (approval timeout). Held action's underlying data becomes stale — при resume данные могут быть неактуальны.
- **Uniqueness:** Bidirectional transition (hold ↔ resume) — уникальная ветка state machine, не terminal. Hold допускается только из APPROVED (до scheduling).
