...
### PRO-08: WB promo execution (RECOMMENDATION-only until P-4 resolved)

- **Назначение:** Обработка промо-решений для Wildberries.
- **Trigger:** `promo_decision` for WB connection.
- **Main path (fallback):** WB promo write API недоступен (P-4 OPEN). Pipeline работает в **recommendation-only режиме**: evaluation выполняется штатно → `promo_decision` создаётся → `participation_mode` принудительно понижается до `RECOMMENDATION` → promo_action **не создаётся**. UI Promo Journal показывает рекомендации с пометкой `wb.promo.write_unavailable`. Оператор выполняет activate/deactivate вручную через ЛК WB.
- **Transition to live:** После разблокировки P-4 снятие override → pipeline создаёт actions согласно policy.
- **Dependencies:** WB promo API (blocker для write). Evaluation pipeline (работает). `promotions.md` — секция «Graceful degradation».
- **Failure risks:** Оператор пропускает рекомендацию → упущенная акция. Mitigation: alert при накоплении unacted recommendations.
- **Uniqueness:** Provider-specific fallback — единственный marketplace с recommendation-only ограничением.
...
- **Main path:** `promo_guard` checks `canonical_promo_product` → `participation_status = PARTICIPATING` + `campaign.status IN (ACTIVE, UPCOMING, FROZEN)` → pricing skipped for this offer → explanation: `promo_active`. FROZEN включён — маркетплейс зафиксировал цену, repricing бессмыслен.
- **Dependencies:** `canonical_promo_product` freshness. Promo sync timing.
- **Failure risks:** Stale promo data → pricing overrides promo price. Promo ended but data not synced → unnecessary block. FROZEN not detected → pricing attempts change of marketplace-locked price.
