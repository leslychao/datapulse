...
- **Основной поток:**
  ...
  4. **ETL:** Post-sync outbox events: CATALOG_SYNC_COMPLETED, PRICES_SYNC_COMPLETED, etc.
...
- **Основной поток:**
  ...
  4. **Pricing:** PRICES_SYNC_COMPLETED / STOCKS_SYNC_COMPLETED → pricing run triggered.
...
- **Основной поток:**
  1. **ETL:** FINANCE_SYNC → extract finance operations → normalize (sign convention: positive=credit, negative=debit) → canonical_finance_entry (UPSERT) → FINANCE_SYNC_COMPLETED.
...
- **Основной поток:**
  ...
  2. **ETL:** PRICES_SYNC → updates canonical_price_snapshot (references canonical_offer).
  3. **ETL:** STOCKS_SYNC → updates canonical_stock_snapshot (references canonical_offer).
  ...
  6. **Analytics:** Materialization respects dependencies: dims before facts.
