...
### Этап 2: Integration module — DONE
...
**Критерий готовности:**
- [x] `POST /api/connections` с `marketplace_type: YANDEX` создаёт connection
- [x] Health probe валидирует токен (через WireMock)
- [x] Rate limit groups зарегистрированы и работают
- [x] Integration tests проходят

### Этап 3: ETL adapters + normalizer — DONE
...
**Критерий готовности:**
- [x] Каждый adapter загружает данные из WireMock stubs
- [x] Normalizer тесты покрывают все маппинги из mapping-spec.md
- [x] AsyncReportCapture: generate → poll → download → parse работает с WireMock
- [x] Campaign fan-out для stocks/returns работает

### Этап 4: ETL sources (10 EventSource) — DONE
...
**Критерий готовности:**
- [x] `EventSourceRegistry.resolve(YANDEX, eventType)` возвращает source для всех 10 event types
- [x] Full sync job для YANDEX connection проходит (с WireMock)
- [x] Sub-source results логируются корректно
- [x] DAG зависимости соблюдаются (CATEGORY_DICT/WAREHOUSE_DICT → PRODUCT_DICT → parallel facts)

### Этап 5: Canonical mapping + materialization — DONE
...
**Критерий готовности:**
- [x] Canonical tables populated with Yandex data (from WireMock)
- [x] ClickHouse materialization includes Yandex data
- [x] `delivery_schema` DBS/EXPRESS visible in analytics
- [x] Data quality checks pass (NULL rates within thresholds)

### Этап 6: Execution adapters (price write) — DONE
...
- Quarantine check logic (TODO Phase 2)
...
**Критерий готовности:**
- [x] Price action для YANDEX проходит lifecycle: PENDING → EXECUTING → SUCCEEDED
- [x] Rate limit 420 → retry → success
- [x] Validation error (price=0) → FAILED
- [x] Reconciliation read after write
...
| Область | Пакет / модуль | Описание |
|---------|---------------|----------|
| API caller + DTOs | `io.datapulse.etl.adapter.yandex` | `YandexApiCaller`, 12 read adapters, `YandexNormalizer`, `AsyncReportCapture` |
| Provider DTOs | `io.datapulse.etl.adapter.yandex.dto` | 42 DTO-класса для десериализации Yandex API responses |
| ETL sources | `io.datapulse.etl.domain.source.yandex` | 10 `EventSource` implementations + `YandexMetadata` utility |
| Execution adapters | `io.datapulse.execution.adapter.yandex` | `YandexPriceWriteAdapter`, `YandexPriceReadAdapter` |
| Execution DTOs | `io.datapulse.execution.adapter.yandex.dto` | `YandexUpdatePricesRequest`, `YandexUpdatePricesResponse`, `YandexPriceOffer`, `YandexPriceValue` |
| Health probe | `io.datapulse.integration.adapter.yandex` | `YandexHealthProbe` |
| Rate limit groups | `io.datapulse.integration.domain.ratelimit.RateLimitGroup` | `YANDEX_DEFAULT`, `YANDEX_CATALOG`, `YANDEX_ORDERS`, `YANDEX_BIDS`, `YANDEX_PRICE_UPDATE`, `YANDEX_REPORTS`, `YANDEX_WAREHOUSES` |
| Credential keys | `io.datapulse.integration.domain.CredentialKeys` | `YANDEX_API_KEY`, `YANDEX_BUSINESS_ID` |

### EventSource → EtlEventType mapping

| EtlEventType | Source class | Adapters used |
|---|---|---|
| `CATEGORY_DICT` | `YandexCategoryDictSource` | `YandexCategoryTreeReadAdapter` |
| `WAREHOUSE_DICT` | `YandexWarehouseDictSource` | `YandexWarehousesReadAdapter` |
| `PRODUCT_DICT` | `YandexProductDictSource` | `YandexCatalogReadAdapter` |
| `PRICE_SNAPSHOT` | `YandexPriceSnapshotSource` | `YandexPriceReadAdapter` (ETL) |
| `INVENTORY_FACT` | `YandexInventoryFactSource` | `YandexStocksReadAdapter` |
| `SUPPLY_FACT` | `YandexSupplyFactSource` | — (stub, no-op) |
| `SALES_FACT` | `YandexSalesFactSource` | `YandexOrdersReadAdapter`, `YandexReturnsReadAdapter` |
| `FACT_FINANCE` | `YandexFinanceFactSource` | `YandexFinanceReportReadAdapter`, `AsyncReportCapture` |
| `PROMO_SYNC` | `YandexPromoSyncSource` | `YandexPromoReadAdapter` |
| `ADVERTISING_FACT` | `YandexAdvertisingFactSource` | `YandexBidsReadAdapter` |

### Cross-module integration points

- **Connection → ETL:** `YandexHealthProbe` saves `businessId` + `campaigns[]` to `marketplace_connection.metadata`. ETL sources parse via `YandexMetadata.parse(ctx.connectionMetadata())`.
- **ETL → Execution:** `ExecutionCredentialResolver.enrichWithYandexMetadata` extracts `businessId` from connection metadata and adds to credentials map for price adapters.
- **Error handling:** `ErrorClassifier` handles Yandex HTTP 420 (rate limit) as `RETRIABLE_RATE_LIMIT`.
- **Gateway registration:** `LivePriceActionGateway` auto-discovers `YandexPriceWriteAdapter` / `YandexPriceReadAdapter` via `List<PriceWriteAdapter>` injection.
