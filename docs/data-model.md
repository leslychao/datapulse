...
| [Pricing](modules/pricing.md) | `price_policy` (versioned, `last_preview_version` for mandatory preview gate, strategy_type: TARGET_MARGIN, PRICE_CORRIDOR, VELOCITY_ADAPTIVE, STOCK_BALANCING, COMPOSITE, COMPETITOR_ANCHOR, MANUAL_OVERRIDE), `price_policy_assignment`, `price_decision` (policy_snapshot, explanation format), `pricing_run` (trigger_type incl. `MANUAL_BULK`, `request_hash`; statuses: PENDING, IN_PROGRESS, COMPLETED, COMPLETED_WITH_ERRORS, FAILED, PAUSED, CANCELLED), `manual_price_lock`, `competitor_match` (workspace_id, marketplace_offer_id, competitor_name, match_method, trust_level), `competitor_observation` (competitor_match_id, competitor_price, observed_at), `pricing_insight` (workspace_id, insight_type, title, body, severity, acknowledged) |
