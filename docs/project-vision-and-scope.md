...
4. **Ozon Advertising** — Performance API требует отдельной OAuth2 регистрации (client_credentials flow) на отдельном хосте (`api-performance.ozon.ru`). Блокирует Phase B extended (Ozon advertising ingestion)
5. **WB Advertising v2→v3 migration** — endpoint `POST /adv/v2/fullstats` отключён; новый `GET /adv/v3/fullstats` требует миграции adapter code (POST→GET). Блокирует Phase B extended (WB advertising ingestion)
