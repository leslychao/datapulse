...
| `ON_HOLD` | Приостановлено оператором **или** blast radius circuit breaker ([Pricing §Aggregate blast radius](pricing.md#aggregate-blast-radius-protection)) | → APPROVED / CANCELLED / SUPERSEDED |
...
| Retriable (transient) | Backoff + retry | HTTP 500, 502, 503, 504, connect timeout, DNS failure, connection refused, TLS handshake failure |
| Uncertain (request sent, no response) | → RECONCILIATION_PENDING | Read timeout, socket timeout после отправки запроса |
| Non-retriable | FAILED немедленно | HTTP 400, 401, 403 (кроме Ozon empty prices edge case), validation error |

**Критерий retriable vs uncertain:** запрос гарантированно не достиг провайдера → retriable. Запрос мог быть обработан → uncertain.

**HTTP 5xx детализация:**

| HTTP Status | Тип | Обоснование |
|-------------|-----|-------------|
| 500 Internal Server Error | Retriable (transient) | Серверная ошибка провайдера; обычно transient |
| 502 Bad Gateway | Retriable (transient) | Upstream failure; transient |
| 503 Service Unavailable | Retriable (transient) | Перегрузка / maintenance |
| 504 Gateway Timeout | Retriable (transient) | Upstream timeout; запрос мог не дойти — safe to retry |

**Network errors (запрос не отправлен):**

| Ошибка | Тип | Обоснование |
|--------|-----|-------------|
| Connect timeout | Retriable (transient) | Соединение не установлено → запрос не достиг провайдера |
| DNS failure | Retriable (transient) | Провайдер не резолвится → запрос не отправлен. При sustained DNS failure → non-retriable после max_attempts |
| Connection refused | Retriable (transient) | Порт не слушает → запрос не отправлен |
| TLS handshake failure | Retriable (transient) | TLS negotiation failed → запрос не отправлен. При persistent failure (expired cert) → non-retriable после max_attempts |
...
**Ozon HTTP-level errors:**

| HTTP Status | Тип | Действие |
|-------------|-----|----------|
| 200 + `updated: true` | Success | SUCCEEDED |
| 200 + `updated: false` | Per-item error | Classify by `errors[].code` (см. выше) |
| 400 Bad Request | Non-retriable | FAILED (invalid payload) |
| 403 Forbidden | Non-retriable | FAILED (auth error / empty prices array) |
| 429 Too Many Requests | Retriable (rate limit) | Backoff + retry |
| 500, 502, 503, 504 | Retriable (transient) | Backoff + retry |

**WB: классификация poll `errorText` (Этап 1 reconciliation):**

| `errorText` | Тип | Действие |
|-------------|-----|----------|
| Empty string | Success | SUCCEEDED (primary evidence) |
| Non-empty, содержит «цена уже установлена» / «price already set» | Non-retriable (idempotent) | SUCCEEDED (price already matches intent) |
| Non-empty, прочие | Non-retriable | FAILED. Конкретная taxonomy WB error messages документируется в [write-contracts.md](../provider-api-specs/write-contracts.md) по мере накопления данных |

**WB: polling ≠ success confirmation:**

Poll result подтверждает, что WB **принял и обработал** upload task — не что цена фактически отображается. Между обработкой task и отражением цены в каталоге существует eventual consistency window. Поэтому poll success → primary evidence, но read-after-write (secondary evidence) необходим для full confirmation (см. §Reconciliation).
...
Action executed → provider response →
  SIMULATED mode: any success → SUCCEEDED (no real API to verify)
  LIVE mode:
    if confirmed (primary evidence positive) → RECONCILIATION_PENDING →
      → deferred read-after-write verification →
      → SUCCEEDED (match) / retry / FAILED (mismatch or timeout)
    if uncertain (timeout, poll empty) → RECONCILIATION_PENDING →
      → deferred read-after-write verification →
      → SUCCEEDED (match) / retry / FAILED (mismatch or timeout)
```

**Принцип:** для LIVE mode любой результат (confirmed / uncertain) проходит через deferred reconciliation. Primary evidence (WB poll ok / Ozon `updated: true`) сохраняется в attempt с `reconciliation_source = IMMEDIATE`, но окончательный SUCCEEDED ставится только после read-after-write verification. Для SIMULATED mode — нет реального API, shadow-state обновляется детерминистически, reconciliation не нужна.
...
| Provider | Поле сравнения | Tolerance | Обоснование |
|----------|---------------|-----------|-------------|
| WB | `sizes[].price` vs `target_price` | **Exact match** (tolerance = 0) | WB prices — integer RUB. Отправляем integer, получаем integer. Расхождение невозможно при корректном write |
| Ozon | `price.price` vs `target_price` | **Exact string match** (tolerance = 0) | Ozon prices — строки без дробной части для RUB. Отправляем `"5000"`, проверяем `"5000.0000"` — сравнение через `BigDecimal.compareTo()` (игнорирует trailing zeros) |

**`price_match` computation:**

```
WB:   price_match = (actual_price == target_price)
Ozon: price_match = (new BigDecimal(actual_price).compareTo(target_price) == 0)
```

При mismatch: `price_match = false`, `actual_price` сохраняется в attempt для аудита.

### Primary-secondary evidence conflict resolution

При расхождении primary (write confirmation) и secondary (read-after-write):

| Сценарий | Действие | SLA разбора |
|----------|----------|-------------|
| Primary = success, secondary = match | SUCCEEDED | — |
| Primary = success, secondary = mismatch | SUCCEEDED + alert `reconciliation.read_mismatch_after_confirmed_write` | 4 часа (бизнес-часы). Вероятная причина: eventual consistency lag у провайдера. Escalation: если мисматч сохраняется > 30 мин при повторном read → manual investigation |
| Primary = success, secondary = read error | SUCCEEDED + alert `reconciliation.read_unavailable` | 8 часов. Read API мог быть временно недоступен. Не блокирует SUCCEEDED |
| Primary = failure | FAILED (независимо от secondary) | — |
| Primary = uncertain (WB poll empty / timeout) | RECONCILIATION_PENDING → deferred checks | Автоматически, до `reconciliation_timeout` (10 мин) |

**Escalation chain:** alert → Grafana → notification channel (Telegram/email) → on-call оператор. Если оператор не реагирует в SLA → повторный alert с повышенным severity. Детали notification routing: [Audit & Alerting](audit-alerting.md).
...
Provider call completed →
  ...
  Ozon: synchronous response (single item per API call)

  WB poll outcomes:
    if poll HTTP error (429, 5xx, timeout) → RECONCILIATION_PENDING (poll failure ≠ write failure)
    if poll success, historyGoods empty (still processing) → RECONCILIATION_PENDING
    if poll success, errorText empty → primary evidence saved as IMMEDIATE, → RECONCILIATION_PENDING (deferred verify)
    if poll success, errorText non-empty → FAILED (non-retriable, see §WB errorText taxonomy)

  Ozon outcomes:
    if updated:true → primary evidence saved as IMMEDIATE, → RECONCILIATION_PENDING (deferred verify)
    if updated:false → classify by error code (see §Классификация ошибок)
```

**WB polling error ≠ write failure.** Если poll-запрос сам получает HTTP-ошибку (429, 5xx, timeout), это не означает, что upload task провалился — это означает, что мы не можем узнать результат. Действие: RECONCILIATION_PENDING → deferred check позже, когда poll API доступен. Не путать с WB upload response `error: true` (write rejection).

**Ozon: single-item execution.** При `prefetchCount=1` consumer обрабатывает один message (= один price_action = один offer) за раз. Ozon API call содержит один item в `prices[]` array. Partial success handling не требуется при текущей архитектуре. При Phase G batching (несколько actions в одном API call для эффективности) — partial success mapping описывается отдельно.
...
**Покрытие pre-execution состояний:**

| Состояние | Safety net | Механизм |
|-----------|-----------|----------|
| `PENDING_APPROVAL` | Expiration job (каждый 1 час) | `created_at + approval_timeout_hours < NOW()` → EXPIRED |
| `APPROVED` | Не требуется | Transient state — всегда бандлится с SCHEDULED + outbox в одной транзакции (см. §APPROVED → SCHEDULED transition) |
| `ON_HOLD` | Зависит от инициатора | **Manual hold:** оператор контролирует, нет автоматического timeout. **Blast radius hold:** pricing run PAUSED → actions ON_HOLD; resume → APPROVED, cancel → CANCELLED (см. [Pricing §Aggregate blast radius](pricing.md#aggregate-blast-radius-protection)) |

**Self-monitoring stuck detector:** если сам scheduled job перестаёт работать (OOM, unhandled exception, pod eviction), stuck states накапливаются тихо. Защита:

- Метрика `execution_stuck_detector_last_run_at` — timestamp последнего успешного запуска
- Alert: `execution_stuck_detector_last_run_at < NOW() - 15 min` → «Stuck detector not running» (critical)
- Expiration job аналогично: метрика `execution_expiration_job_last_run_at`, alert при > 2 часа без запуска
- Оба job'а логируют `log.info("Stuck detector completed: checked={}, escalated={}")` при каждом запуске для visibility в логах
...
| Isolation level | PostgreSQL READ COMMITTED (default). Достаточно для single-row CAS: `UPDATE ... WHERE id = :id AND status = :expected` захватывает row-level exclusive lock, предотвращая concurrent update |

**Race: worker vs stuck-state detector.** При медленном provider call (близко к EXECUTING TTL = 5 мин) stuck-state detector может эскалировать EXECUTING → RECONCILIATION_PENDING до завершения worker'а. Когда worker завершает и пытается CAS (EXECUTING → RETRY_SCHEDULED или EXECUTING → SUCCEEDED), CAS fails — intent worker'а потерян.

Это **осознанный trade-off**: stuck-state detector приоритетнее, чем retry intent. Обоснование:

- Если worker ещё работает спустя 5 мин — скорее всего, provider call завис (не retriable ситуация)
- RECONCILIATION_PENDING проверит фактическое состояние через read API — это безопаснее, чем retry
- Worker логирует `log.warn("CAS conflict: action={}, attempted={}, current={}")` для диагностики
- Метрика `execution_cas_conflict_total{transition="..."}` отслеживает частоту конфликтов; alert при sustained rate > 0
...
| `APPROVED → ON_HOLD` | Blast radius circuit breaker ([Pricing §Aggregate blast radius](pricing.md#aggregate-blast-radius-protection)): pricing run PAUSED → все APPROVED actions run → ON_HOLD | system |
| `APPROVED → CANCELLED` | Manual cancel | operator |
| `APPROVED → SUPERSEDED` | Более свежий action (pre-execution) | system |
| `ON_HOLD → APPROVED` | Manual resume **или** pricing run resume (blast radius override) | operator / system |
| `ON_HOLD → CANCELLED` | Manual cancel **или** pricing run cancel (blast radius) | operator / system |
...
Для ON_HOLD → APPROVED (resume): аналогичная транзакция — resume бандлит все три шага:

```
@Transactional:
  1. CAS: ON_HOLD → APPROVED
  2. INSERT outbox_event (type: PRICE_ACTION_EXECUTE, aggregate_id: price_action.id)
  3. CAS: APPROVED → SCHEDULED
```

**Инвариант:** action НЕ остаётся в APPROVED без outbox event. Переход APPROVED → SCHEDULED всегда сопровождается INSERT outbox_event. Это гарантируется для всех путей: initial approval (PENDING_APPROVAL → APPROVED), manual approval (SEMI_AUTO), и resume (ON_HOLD → APPROVED).
...
**Почему `RETRY_SCHEDULED` в deferred-группе:** хотя в RETRY_SCHEDULED provider call текущей попытки уже завершён (ответ получен), action ещё «в процессе» — pending retry. Immediate supersede отменил бы запланированный retry, но action мог уже частично повлиять на провайдера (uncertain timeout → RETRY_SCHEDULED). Консервативный подход: дождаться завершения всего цикла retry, затем заменить через deferred queue.
...
**Атомарность immediate vs deferred решения:**

Выбор между immediate supersede и deferred queue выполняется **в одной транзакции** с созданием нового action:

```
@Transactional:
  1. SELECT status FROM price_action
     WHERE marketplace_offer_id = :offerId AND execution_mode = :mode
       AND status NOT IN (terminal states)
     FOR UPDATE  -- row-level lock предотвращает concurrent transition
  2. if found AND status IN (PENDING_APPROVAL, APPROVED, ON_HOLD, SCHEDULED):
       → CAS: current → SUPERSEDED
       → INSERT new price_action
  3. if found AND status IN (EXECUTING, RETRY_SCHEDULED, RECONCILIATION_PENDING):
       → UPSERT deferred_action
  4. if not found:
       → INSERT new price_action
```

`SELECT ... FOR UPDATE` на текущем active action предотвращает race: пока транзакция держит lock, concurrent completion (EXECUTING → SUCCEEDED) блокируется. Если active action завершается до начала транзакции — `SELECT` не находит его (step 4), новый action создаётся напрямую.
...
|---------|----------|----------|
| Action creation | Partial unique index `idx_price_action_active_offer_{live,simulated}` | Дубль невозможен: INSERT нарушит unique constraint → `DataIntegrityViolationException` → catch + skip (action уже существует) |
| Attempt execution | CAS claim: `SCHEDULED → EXECUTING WHERE status = SCHEDULED` | Single-winner: concurrent consumers → только один succeeds, остальные CAS fail → skip |
| Outbox delivery | At-least-once publish + idempotent consumer (CAS) | Duplicate message → consumer пытается CAS `SCHEDULED → EXECUTING` → fails (already EXECUTING) → skip. Корректно при: outbox poller publishes → crash до mark PUBLISHED → re-publish → consumer получает дубль |
| Provider call | Last-write-wins (natural idempotency) | Повторный вызов с той же ценой = тот же результат. Цена — stateless target, не delta |

### Provider-level idempotency — обоснование «last-write-wins»

Price write — это **set target** (задать абсолютную цену), не **adjust** (изменить на дельту). Повторный `POST /api/v2/upload/task` с `price: 5000` для того же `nmID` эквивалентен одному вызову — цена будет 5000. Аналогично для Ozon `POST /v1/product/import/prices` с `price: "5000"`.

| Provider | Поведение при повторе | Confidence |
|----------|-----------------------|------------|
| WB | `alreadyExists: true` при дубле upload task (dedup window неизвестен, assumed). Если dedup window истёк — принимается как новый task, результат тот же (last-write-wins) | assumed |
| Ozon | Нет explicit idempotency mechanism. `updated: true` при повторе с той же ценой. Per-product rate limit (10/hour) — implicit throttle | confirmed-docs |

**Риск double-write:** Минимален для price setting (idempotent by nature). Потенциальная проблема: per-product rate limit Ozon (10 updates/hour) — повторные вызовы расходуют лимит. Митигация: per-product sliding window counter в Redis (см. §Rate limit координация).
...
**Defence-in-depth assertion:** каждый gateway **обязан** проверять `execution_mode` в начале `execute()`:

```
LivePriceActionGateway.execute(action):
  assert action.executionMode == LIVE : "Live gateway received non-LIVE action"

SimulatedPriceActionGateway.execute(action):
  assert action.executionMode == SIMULATED : "Simulated gateway received non-SIMULATED action"
```

При violation → `IllegalStateException` → action переходит в FAILED (non-retriable) + critical alert. Это предотвращает катастрофическую ошибку маршрутизации (SIMULATED action попадает в live write).
...
| Offer archived/deleted | Orphan row остаётся; cleanup при следующем manual reset или periodic cleanup job |
| Retention | Бессрочно (lightweight — одна строка per offer). Periodic cleanup (Phase G): `DELETE FROM simulated_offer_state WHERE marketplace_offer_id NOT IN (SELECT id FROM marketplace_offer WHERE status = 'ACTIVE')` |
...
| `price_action_state_transition` | Audit trail: все CAS state transitions с actor, reason, timestamp |
...
price_action_state_transition:
  ...
  actor_user_id             BIGINT FK → app_user (nullable; NULL для system transitions)
  reason                    TEXT (nullable)
  created_at                TIMESTAMPTZ DEFAULT now()
```

Каждый CAS-переход в `ActionService` записывает строку в эту таблицу. Используется для отображения timeline в UI и аудита.
...
| POST | `/api/workspaces/{workspaceId}/actions/{actionId}/reject` | PRICING_MANAGER, ADMIN, OWNER | Reject: PENDING_APPROVAL → CANCELLED. Body: `{ cancelReason }`. Bulk: `POST /api/workspaces/{workspaceId}/actions/bulk-reject` Body: `{ actionIds: [...], cancelReason }` |
