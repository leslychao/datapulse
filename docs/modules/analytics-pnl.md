...
### Display conventions (backend → frontend contract)

| Measure | Signed in DB | API response | Пояснение |
|---------|-------------|-------------|-----------|
| `totalCostsAmount` | Negative (sum of 8 cost measures) | **Positive** (`.abs()`) | Backend negates for display; frontend shows as-is |
| `costBreakdown[].amount` | Negative per category | **Positive** (`.abs()`) | Individual cost items also absolute |
| `totalCostsAmount` в trend | Negative | **Positive** (SQL: `-(sum(...))`) | Chart expects positive values above zero axis |
| `reconciliationRatio` | — | `|residual| / |net_payout| × 100` | Uses absolute values for both numerator and denominator |
| `revenue`, `compensation`, `cogs`, `advertising` | Positive | Positive | No transformation needed |
| `refundAmount`, `marketplacePnl`, `fullPnl` | Signed | Signed (as-is) | Frontend handles sign display |
...
`reconciliation_ratio = |residual| / |net_payout| × 100` — метрика качества маппинга (в процентах). Denominator = `net_payout` (не `revenue`). Numerator и denominator используют абсолютные значения для корректной обработки edge cases. Ожидаемые baselines:
...
Phase B: TTL не установлен — объёмы данных малы (единицы аккаунтов). Canonical layer (PostgreSQL) = source of truth, re-materialization восстановит любой период.

Phase G retention targets:

| Тип таблицы | Retention | TTL expression | Обоснование |
|-------------|-----------|----------------|-------------|
| `fact_finance` | 36 месяцев | `TTL finance_date + INTERVAL 36 MONTH` | Покрытие 3-year P&L history, tax reporting |
| `fact_sales` | 36 месяцев | `TTL sale_date + INTERVAL 36 MONTH` | Согласован с fact_finance |
| `fact_orders` | 24 месяца | `TTL order_date + INTERVAL 24 MONTH` | Operational, не P&L-critical |
| `fact_returns` | 24 месяца | `TTL return_date + INTERVAL 24 MONTH` | Operational |
| `fact_price_snapshot` | 12 месяцев | `TTL captured_date + INTERVAL 12 MONTH` | High-volume snapshots |
| `fact_inventory_snapshot` | 12 месяцев | `TTL captured_date + INTERVAL 12 MONTH` | High-volume snapshots |
| `fact_product_cost` | Без TTL | — | SCD2, малый объём |
| `fact_advertising` | 36 месяцев | `TTL ad_date + INTERVAL 36 MONTH` | Aligned with fact_finance для ретроактивного P&L recalc |
| `dim_*` | Без TTL | — | Малый объём, slowly changing |
| `mart_*` | Без TTL | — | Пересчитываются из facts, TTL наследуется от source facts |

Реализация: `ALTER TABLE <table> MODIFY TTL <expression>`. Применяется через ClickHouse migration script при переходе на Phase G.
...
Grain: одна строка per price observation. Grain key = ORDER BY tuple `(workspace_id, product_id, captured_at)`.
...
Grain: одна строка per stock observation. Grain key = ORDER BY tuple `(workspace_id, product_id, warehouse_id, captured_at)`.
...
Полная спецификация — см. §dim_advertising_campaign ниже (provider-specific sources, nullable types).

#### fact_advertising (Phase B extended)

Полная спецификация — см. §fact_advertising ниже (grain caveats, dedup key, provenance, retention).
...
Grain: workspace × source_platform × seller_sku × period × attribution_level. Содержит rollup из mart_posting_pnl + PRODUCT-level + ACCOUNT-level entries.
...
Grain: workspace × source_platform × seller_sku × period. Phase B.
...
Grain: одна строка per рекламная кампания (workspace_id × source_platform × campaign_id).
...
**Grain:** `workspace_id × source_platform × campaign_id × ad_date × marketplace_sku` (product-level daily). Для Ozon (если product-level недоступен): `workspace_id × source_platform × campaign_id × ad_date` (campaign-level daily) с `marketplace_sku = ''` (пустая строка, не NULL — ClickHouse sorting key не допускает Nullable).
...
**Dedup key:** `(workspace_id, source_platform, campaign_id, ad_date, marketplace_sku)`. ReplacingMergeTree с `ver` — upsert при re-materialization.
...
Monthly partitioning по `finance_date` — оптимально для P&L queries (фильтрация по period). `workspace_id` не в PARTITION BY (малое количество workspaces в MVP). ReplacingMergeTree merge работает within partition.
...
Grain = canonical_finance_entry (одна composite row per entry). Sorting key: `(workspace_id, source_platform, entry_id)`. `entry_id` = PK canonical_finance_entry (bigint, globally unique). `finance_date` исключён из sorting key для предотвращения ghost rows при re-materialization. Posting-level P&L строится в `mart_posting_pnl` через `GROUP BY posting_id`.
...
**Partial failure (materializer-level):**

Если materializer `N` завершается ошибкой, оставшиеся materializer-ы фазы **не выполняются** — pipeline прерывается. Состояние:
- Materializer-ы, выполненные до `N` — уже записали данные (ReplacingMergeTree upsert, idempotent).
- Materializer `N` и далее — не выполнены. Marts могут быть stale.

Recovery: worker логирует ошибку + публикует `MATERIALIZATION_FAILED` alert. Следующий scheduled или incremental trigger **перезапускает всю цепочку** (DIMENSION → FACT → MART). Благодаря `ReplacingMergeTree(ver)` повторная запись безопасна — latest `ver` wins.

**TRUNCATE + failed INSERT (full re-materialization):**

Для full re-materialization используется **двухфазный swap**:
1. `CREATE TABLE <table>_staging AS <table>` (пустая staging-таблица с идентичной схемой)
2. Batch INSERT в `<table>_staging` из canonical (PostgreSQL) или facts (для marts)
3. `EXCHANGE TABLES <table> AND <table>_staging` (атомарная операция ClickHouse)
4. `DROP TABLE <table>_staging`

Если шаг 2 падает — production-таблица **не затронута** (staging отбрасывается). Если шаг 3 падает — retry безопасен (EXCHANGE идемпотентен при одинаковых данных). Fallback: если EXCHANGE недоступен (older ClickHouse) — `RENAME TABLE <table> TO <table>_old, <table>_staging TO <table>`, затем `DROP TABLE <table>_old`.

**Monitoring:**

| Метрика | Описание |
|---------|----------|
| `materialization_run_status` | `COMPLETED` / `PARTIAL_FAILURE` / `FAILED` per run |
| `materialization_duration_seconds` | Время выполнения per materializer |
| `materialization_rows_written` | Количество записанных строк per materializer |

Alert: `PARTIAL_FAILURE` или `FAILED` → `alert_event` с `blocks_automation = false` (данные stale, но не corrupt). Два consecutive failures → `blocks_automation = true`.
...
ClickHouse — read-only analytics store, не source of truth. При его недоступности core operations (tenancy, connections, ETL canonical layer) продолжают работать. Деградируют: аналитические UI-страницы и pricing signal assembler.
...
**Pricing pipeline dependency:**

Signal assembler читает derived signals (margin, velocity, return rate) из ClickHouse marts. При недоступности:

1. Signal assembler бросает `AnalyticsUnavailableException`
2. Pricing run переходит в `FAILED` с `reason: ANALYTICS_UNAVAILABLE`
3. **Не создаёт** `blocks_automation = true` alert (transient failure, не data quality issue)
4. Retry: следующий scheduled pricing trigger (или ETL_SYNC_COMPLETED event) перезапускает run
5. Stale data guard (`last_success_at > 24h`) срабатывает независимо — если ClickHouse down > 24h, финансовые данные станут stale → automation блокируется через стандартный data quality path

Кэширование сигналов **не применяется**: stale signals опаснее отсутствия решения. Pricing pipeline предпочитает skip-and-retry вместо stale-and-decide.

**UI-уровень (frontend):**

При получении `503` от analytics endpoints:
- Показать banner: `{{ 'analytics.unavailable.banner' | translate }}` — «Аналитика временно недоступна. Данные обновятся автоматически.»
- Показать timestamp последнего успешного ответа (из TanStack Query cache `dataUpdatedAt`)
- Auto-retry через TanStack Query `refetchInterval` (30s при ошибке)
- Навигация, настройки, подключения — **не деградируют** (PostgreSQL-backed)

**Health check:**

`/actuator/health` включает ClickHouse health indicator:
- `UP`: query `SELECT 1` < 5s
- `DOWN`: timeout или connection refused

Health status `DOWN` для ClickHouse **не переводит** весь сервис в `DOWN` — ClickHouse = non-critical dependency для API liveness. Readiness probe включает ClickHouse — новые analytics requests не роутятся на instance с DOWN ClickHouse до recovery.

**Materialization при ClickHouse down:**

Incremental materializer при ClickHouse connection failure: retry 3× с exponential backoff (1s, 2s, 4s). Если все retries исчерпаны — materialization run помечается `FAILED`, данные остаются в canonical (PostgreSQL). При восстановлении ClickHouse — следующий scheduled или incremental trigger восстановит consistency. Daily full re-materialization — ultimate safety net (bounds max staleness 24h после recovery).
...
> **Фильтрация по маркетплейсу:** P&L endpoints используют `sourcePlatform` (значения: `ozon`, `wb`), а не `connectionId`. Обоснование: в рамках workspace действует constraint `UNIQUE (workspace_id, marketplace_type, external_account_id) WHERE status <> 'ARCHIVED'` — фактически одно активное подключение per marketplace per workspace. `sourcePlatform` — достаточный и стабильный дискриминатор.

| Method | Path | Roles | Описание |
|--------|------|-------|----------|
| GET | `/api/workspaces/{workspaceId}/analytics/pnl/summary` | Any role | P&L aggregated summary за период. Filters: `?period=YYYY-MM&sourcePlatform=...&from=...&to=...`. Response: aggregated P&L components с delta vs previous period, cost breakdown, reconciliation ratio |
| GET | `/api/workspaces/{workspaceId}/analytics/pnl/by-product` | Any role | P&L по продуктам. Paginated. Filters: `?period=YYYY-MM&sourcePlatform=...&sellerSkuId=...&search=...`. Sort: any P&L column, direction ASC/DESC. **NB:** frontend отправляет `period` как `YYYY-MM`, бэкенд конвертирует в UInt32 `YYYYMM` через `PnlFilter.periodAsInt()` |
| GET | `/api/workspaces/{workspaceId}/analytics/pnl/by-posting` | Any role | P&L по отправкам. Paginated. Filters: `?sourcePlatform=...&from=...&to=...&sellerSkuId=...&search=...`. Sort: any posting column, direction ASC/DESC |
...
| GET | `/api/workspaces/{workspaceId}/analytics/pnl/trend` | Any role | P&L trend by period (daily/weekly/monthly). Filters: `?sourcePlatform=...&from=...&to=...&period=YYYY-MM&granularity=MONTHLY`. Data source: monthly → `mart_product_pnl`, daily/weekly → `mart_posting_pnl` |

### Inventory intelligence

| Method | Path | Roles | Описание |
|--------|------|-------|----------|
| GET | `/api/workspaces/{workspaceId}/analytics/inventory/overview` | Any role | Inventory overview: total SKUs, stock-out risk distribution, frozen capital. Фильтры отсутствуют (workspace-wide) |
...
| Method | Path | Roles | Описание |
|--------|------|-------|----------|
| GET | `/api/workspaces/{workspaceId}/analytics/returns/summary` | Any role | Return rate summary. Filter: `?sourcePlatform=...&period=YYYY-MM` |
| GET | `/api/workspaces/{workspaceId}/analytics/returns/by-product` | Any role | Per-product return analysis. Paginated. Filter: `?sourcePlatform=...&period=YYYY-MM&search=...`. Includes return_rate_pct, top_reason |
| GET | `/api/workspaces/{workspaceId}/analytics/returns/trend` | Any role | Return rate trend over time. Filter: `?sourcePlatform=...&from=...&to=...&granularity=DAILY|WEEKLY|MONTHLY` |
