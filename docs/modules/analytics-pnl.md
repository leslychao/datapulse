...
Все measures в fact_finance хранятся с **естественными знаками** (signed values), идентичными canonical layer:

- **Positive** = credit to seller (revenue, compensation, commission refund on returns)
- **Negative** = debit from seller (costs, penalties, revenue reversal on returns)

Materializer копирует canonical measures **as-is**, без abs()-трансформации. Нормализация знаков — ответственность ETL normalizer (описана в [etl-pipeline.md](etl-pipeline.md) §Sign conventions): WB debit-поля умножаются на −1, Ozon signs сохраняются.
