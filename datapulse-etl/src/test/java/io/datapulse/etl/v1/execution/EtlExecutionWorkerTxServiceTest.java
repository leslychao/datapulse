...
class EtlExecutionWorkerTxServiceTest {
  ...
  void retrySchedulingRespectsMaxAttemptsAndWritesOutbox() {
    ...
    EtlExecutionWorkerTxService service = new EtlExecutionWorkerTxService(state, registry, raw, channel, outbox, new EtlExecutionPayloadCodec());
    ...
    when(channel.send(any())).thenThrow(new LocalRateLimitBackoffRequiredException("slow", 1000));
  ...
  void successPathDeletesRawByRequestIdThenInvokesIngest() {
    ...
    var inOrder = inOrder(raw, channel, state);
    inOrder.verify(raw).deleteByRequestId("raw_table", "req-ok");
    inOrder.verify(channel).send(any());
