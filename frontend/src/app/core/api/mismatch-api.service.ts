...
export class MismatchApiService {
  ...
  getSummary(workspaceId: number): Observable<MismatchSummary> {
    return this.http.get<MismatchSummary>(`${this.base}/workspace/${workspaceId}/mismatches/summary`);
  }
  ...
  list(workspaceId: number, filter: MismatchFilter, page: number, size: number, sort = 'detectedAt,desc'): Observable<Page<Mismatch>> {
    ...
    return this.http.get<Page<Mismatch>>(`${this.base}/workspace/${workspaceId}/mismatches`, { params });
  }

  getDetail(workspaceId: number, mismatchId: number): Observable<MismatchDetail> {
    return this.http.get<MismatchDetail>(`${this.base}/workspace/${workspaceId}/mismatches/${mismatchId}`);
  }

  acknowledge(workspaceId: number, mismatchId: number): Observable<void> {
    return this.http.post<void>(`${this.base}/workspace/${workspaceId}/mismatches/${mismatchId}/acknowledge`, null);
  }

  resolve(workspaceId: number, mismatchId: number, body: { resolution: string; note: string }): Observable<void> {
    return this.http.post<void>(`${this.base}/workspace/${workspaceId}/mismatches/${mismatchId}/resolve`, body);
  }

  retry(workspaceId: number, actionId: number): Observable<void> {
    return this.http.post<void>(`${this.base}/workspace/${workspaceId}/actions/${actionId}/retry`, { retryReason: 'mismatch investigation' });
  }
}
