  ...
  template: `
    <div class="flex flex-col items-center gap-4 text-center">
      ...
      @switch (icon) {
        @case ('spinner') {
          <div
            class="h-6 w-6 animate-spin rounded-full border-2 border-[var(--border-default)] border-t-[var(--accent-primary)]"
          ></div>
        }
        @case ('success') {
          <div
            class="flex h-10 w-10 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--status-success)_12%,transparent)]"
          >
        ...
        @case ('error') {
          <div
            class="flex h-10 w-10 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--status-error)_12%,transparent)]"
          >
        ...
        @case ('info') {
          <div
            class="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--accent-subtle)]"
          >
        ...
        @case ('warning') {
          <div
            class="flex h-10 w-10 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--status-warning)_12%,transparent)]"
          >
...
export class StatusMessageComponent {
  @Input() icon: 'spinner' | 'success' | 'error' | 'info' | 'warning' = 'spinner';
  @Input() title = '';
  @Input() description = '';
  @Input() actionLabel = '';
