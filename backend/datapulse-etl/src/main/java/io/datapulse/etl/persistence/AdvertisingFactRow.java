...
/**
 * Write model for {@code fact_advertising} in ClickHouse.
 * One row per campaign × date × marketplace_sku.
 */
public record AdvertisingFactRow(
  ...
  public static AdvertisingFactRow fromOzon(
    ...
    BigDecimal cpc = clicks > 0
        ? spend.divide(BigDecimal.valueOf(clicks), 2, java.math.RoundingMode.HALF_UP)
        : BigDecimal.ZERO;
