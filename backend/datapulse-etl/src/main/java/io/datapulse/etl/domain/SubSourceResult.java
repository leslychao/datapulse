...
public record SubSourceResult(
  ...
  public SubSourceResult(
      String sourceId, EventResultStatus status, String lastCursor,
      int pagesProcessed, int recordsProcessed, int recordsSkipped,
      List<String> errors) {
    this(sourceId, status, lastCursor, pagesProcessed, recordsProcessed,
        recordsSkipped, errors, Set.of());
  }

  public boolean isSuccess() {
    return status == EventResultStatus.COMPLETED
        || status == EventResultStatus.COMPLETED_WITH_ERRORS;
  }

  public static SubSourceResult success(
      String sourceId, int pagesProcessed, int recordsProcessed) {
  ...
  public static SubSourceResult successWithWarnings(
      ...
      Set<String> unmappedTypeNames) {
    return new SubSourceResult(sourceId, EventResultStatus.COMPLETED,
        null, pagesProcessed, recordsProcessed, 0, List.of(), unmappedTypeNames);
  }
  ...
  public static SubSourceResult failed(
      String sourceId, String error, String lastCursor) {
    return new SubSourceResult(sourceId, EventResultStatus.FAILED,
        lastCursor, 0, 0, 0, List.of(error));
  }

  public static SubSourceResult partial(
      String sourceId, String lastCursor,
