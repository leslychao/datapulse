...
@DisplayName("DimWarehouseMaterializer")
class DimWarehouseMaterializerTest {
  ...
  @InjectMocks
  private DimWarehouseMaterializer materializer;

  @Test
  @DisplayName("should report table name as 'dim_warehouse'")
  void should_returnTableName() {
    assertThat(materializer.tableName()).isEqualTo("dim_warehouse");
  }
  ...
  class Full {
    ...
    void should_useSwapAndBatch_when_fullRun() {
      ...
      verify(jdbc).fullMaterializeWithSwap(eq("dim_warehouse"), any());
    }
  }
  ...
  class Incremental {
    ...
    @Test
    @DisplayName("should skip when no warehouses affected by job")
